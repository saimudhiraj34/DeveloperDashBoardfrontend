...
const Notes = () => {
  ...
  const handleSave = async() => {
    ...
    try {
      const res = await fetch("http://localhost:3000/note/add_title", {
        method: "POST",
        headers: {
  ...
  const handleSavedes =async(idx,id) => {
    ...
    const token = localStorage.getItem("token");
    try {
      const res = await fetch("http://localhost:3000/note/add_ans", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
  ...
  const handleDelete = async (id) => {
    const token = localStorage.getItem("token");
    try {
      const res = await fetch(`http://localhost:3000/note/delete/${id}`, {
        method: "Delete",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`, 
